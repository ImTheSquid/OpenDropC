...
typedef struct opendrop_client_data_s {
    ...
    struct opendrop_client_data_s *next;
...
// Args:
// - client: OpenDrop client
// - data_list: A linked list of pointers to client data that will be sent
...
// Returns: 0 on success, >0 on error
int opendrop_client_ask(const opendrop_client *client, const opendrop_client_data *data_list, size_t data_arr_len, bool is_url, const unsigned char *icon, size_t icon_len);
